...
export default function SettingsPage() {
  ...
  const { data: initialSettings, isLoading } = useDoc<SystemSettings>(settingsDocRef);
  ...
  const enableGeolocation = watch('enableGeolocation');

   useEffect(() => {
    if (initialSettings) {
      reset(initialSettings);
    } else if (!isLoading) {
      // If no settings found, reset with default values
      reset({
        ...
        companyLatitude: 30.0444,
        companyLongitude: 31.2357,
        allowedRadiusMeters: 200,
        qrCodeLifespan: 15,
      });
    }
  }, [initialSettings, isLoading, reset]);
  ...
  if (isLoading) {
      return (
        <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
            <h2 className="text-3xl font-bold tracking-tight font-headline">إعدادات النظام</h2>
            <SettingsSkeleton />
        </div>
      )
  }
        ...
        <Card>
          ...
          <CardContent className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 border rounded-lg bg-secondary/30">
                <div>
                  ...
                  <Input id="companyStartTime" type="time" {...register('companyStartTime')} />
                  {errors.companyStartTime && <p className="text-sm text-destructive mt-1">{errors.companyStartTime.message}</p>}
                </div>
                <div>
                  <Label htmlFor="companyEndTime">وقت الانصراف العام</Label>
                  <Input id="companyEndTime" type="time" {...register('companyEndTime')} />
                  {errors.companyEndTime && <p className="text-sm text-destructive mt-1">{errors.companyEndTime.message}</p>}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 border rounded-lg bg-secondary/30 space-y-4">
                 <div className="flex items-center space-x-2 space-x-reverse">
                    ...
                    />
                    <Label htmlFor="enableGeolocation">تفعيل التحقق من الموقع الجغرافي</Label>
                 </div>

                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <Label htmlFor="qrCodeLifespan">مدة صلاحية رمز QR (بالثواني)</Label>
                        <Input id="qrCodeLifespan" type="number" {...register('qrCodeLifespan')} />
                        {errors.qrCodeLifespan && <p className="text-sm text-destructive mt-1">{errors.qrCodeLifespan.message}</p>}
                    </div>
                    <fieldset disabled={!enableGeolocation} className="contents">
                        <div>
                            <Label htmlFor="allowedRadiusMeters">نطاق الحضور المسموح (بالمتر)</Label>
                            <Input id="allowedRadiusMeters" type="number" {...register('allowedRadiusMeters')} />
                            {errors.allowedRadiusMeters && <p className="text-sm text-destructive mt-1">{errors.allowedRadiusMeters.message}</p>}
                        </div>
                    </fieldset>
                 </div>
              </div>
            ...
            {/* Geolocation Settings */}
             <fieldset disabled={!enableGeolocation}>
                <div className={cn("space-y-4", !enableGeolocation && "opacity-50")}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 border rounded-lg bg-secondary/30">
                    ...
                    <Input id="companyLatitude" type="number" step="any" {...register('companyLatitude')} />
                    {errors.companyLatitude && <p className="text-sm text-destructive mt-1">{errors.companyLatitude.message}</p>}
                    </div>
                    <div>
                    <Label htmlFor="companyLongitude">خط الطول (Longitude)</Label>
                    <Input id="companyLongitude" type="number" step="any" {...register('companyLongitude')} />
                    {errors.companyLongitude && <p className="text-sm text-destructive mt-1">{errors.companyLongitude.message}</p>}
                    </div>
            ...
            <div className="space-y-6">
                 ...
                 <div className="space-y-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        <div className="flex items-center gap-2">
                            <Input id="gracePeriod" type="number" className="w-32" {...register('gracePeriod')} />
                            <span>دقيقة</span>
                        </div>
                        <Controller
                            control={control}
                            ...
                            render={({ field }) => (
                            <RadioGroup onValueChange={field.onChange} defaultValue={field.value} className="flex gap-4 items-center" >
                                <Label className="flex items-center gap-2 border p-2 px-3 rounded-md cursor-pointer has-[input:checked]:bg-primary has-[input:checked]:text-primary-foreground">
                                    <RadioGroupItem value="daily" id="r-daily" /> يوميًا
                                </Label>
                                <Label className="flex items-center gap-2 border p-2 px-3 rounded-md cursor-pointer has-[input:checked]:bg-primary has-[input:checked]:text-primary-foreground">
                                    <RadioGroupItem value="monthly" id="r-monthly" /> شهريًا
                                </Label>
                            </RadioGroup>
                            )}
                        />
                    </div>
                    {errors.gracePeriod && <p className="text-sm text-destructive mt-1">{errors.gracePeriod.message}</p>}
                ...
                <div className="space-y-4">
                <Label>قواعد الخصم من التأخير (يتم التطبيق على التأخير بعد فترة السماح)</Label>
                    ...
                    <div key={field.id} className="flex flex-col gap-4 p-4 border rounded-lg bg-secondary/50">
                        <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4">
                            <div className="grid gap-2 flex-1 w-full">
                                ...
                                <div className="flex items-center gap-2">
                                <Input type="number" {...register(`deductionRules.${index}.delayMinutes`)} className="w-24" />
                                <span>دقيقة</span>
                                </div>
                                {errors.deductionRules?.[index]?.delayMinutes && <p className="text-sm text-destructive">{errors.deductionRules?.[index]?.delayMinutes?.message}</p>}
                            </div>
                            
                            <div className="grid gap-2 flex-1 w-full">
                                <Label>يتم خصم</Label>
                                <div className="flex items-center gap-2">
                                    <Input type="number" {...register(`deductionRules.${index}.deductionValue`)} className="w-24" />
                                    <Controller
                                        control={control}
                                        name={`deductionRules.${index}.deductionType`}
                                        render={({ field: selectField }) => (
                                            <Select value={selectField.value} onValueChange={selectField.onChange}>
                                                <SelectTrigger className="w-[120px]"><SelectValue placeholder="نوع الخصم" /></SelectTrigger>
                                                <SelectContent>
                                ...
                                {errors.deductionRules?.[index]?.deductionValue && <p className="text-sm text-destructive">{errors.deductionRules?.[index]?.deductionValue?.message}</p>}
                            </div>

                            <Button type="button" variant="destructive" size="icon" onClick={() => remove(index)} className="shrink-0 mt-4 sm:mt-0">
                ...
                <Button type="button" variant="outline" onClick={() => append({ delayMinutes: 60, deductionType: 'hours', deductionValue: 2 })}>
